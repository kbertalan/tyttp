...
export
hEcho : Applicative m
  => (step : Step me p h1 fn s h2 a b)
  -> m $ Step me p h1 fn s h2 a $ fn step.request.method a
hEcho step = pure $ map (const step.request.body) step

export
hMapRequest : Applicative m
  => (a -> a')
  -> Step me h1 p TyTTP.Request.simpleBody s h2 a b
  -> m $ Step me h1 p TyTTP.Request.simpleBody s h2 a' b
hMapRequest f step = pure $ mapFst f step

export
hMapResponse : Applicative m
  => (b -> b')
  -> Step me h1 p fn s h2 a b
  -> m $ Step me h1 p fn s h2 a b'
...
hParseRequest : MonadError e m
  => (String -> m a')
  -> Step me h1 p TyTTP.Request.simpleBody s h2 String b
  -> m $ Step me h1 p TyTTP.Request.simpleBody s h2 a' b
